...
export default function ChatHeader({ 
  ...
  return (
    <header className="px-4 py-2 flex items-center justify-between shadow-sm z-10 shrink-0 h-[60px] bg-white dark:bg-[#212121] border-b border-gray-100 dark:border-black">
      <div className="flex items-center gap-4 flex-1 min-w-0">
        <Link href="/chats" className="md:hidden text-gray-500"><ArrowLeft /></Link>
        ...
        <div className="flex flex-col min-w-0">
          <h2 className="font-bold text-[16px] leading-tight truncate dark:text-white">
            {chatName || "Chat"} 
            <CheckCircle size={12} className="text-accent inline-block ml-1 fill-white" />
          </h2>
          <span className="text-[13px] truncate text-gray-500">
      ...
      <div className="flex gap-4 text-gray-500 items-center flex-shrink-0">
        {isSearchOpen ? (
          <div className="flex items-center bg-gray-100 dark:bg-[#1c1c1d] rounded-full px-3 py-1 animate-in fade-in slide-in-from-right-2 duration-200">
            <input 
              autoFocus 
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              placeholder="Search..."
              className="bg-transparent outline-none text-xs w-24 dark:text-white"
            />
