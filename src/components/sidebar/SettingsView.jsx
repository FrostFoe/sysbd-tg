...
export default function SettingsView() {
  const { 
    user, accentColor, updateAccentColor, setIsQRModalOpen 
  } = useUI();
  const { user: authUser } = useAuth();

  return (
    <div className="flex flex-col">
      <div className="flex flex-col items-center py-6 border-b border-default">
        ...
        <h2 className="text-xl font-bold text-primary">{authUser?.username || authUser?.name}</h2>
        <button 
      ...
      <div className="flex flex-col py-2">
        <button className={`flex items-center gap-4 px-6 py-3 transition hover:bg-hover`}>
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-3 gap-1">
          {[1,2,3,4,5,6].map(i => (
            <div key={i} className="aspect-square bg-input rounded-md overflow-hidden hover:opacity-80 cursor-pointer transition">
