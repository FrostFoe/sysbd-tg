    ...
    <div className="flex flex-col">
      {chats.map((chat) => (
        <Link 
          ...
          className={`flex items-center gap-3 px-3 py-2 cursor-pointer transition select-none ${activeChatId === chat.$id ? 'bg-accent text-white' : darkMode ? 'hover:bg-[#2c2c2e]' : 'hover:bg-gray-100'}`}
        >
          ...
          <div className={`flex-1 min-w-0 border-b pb-2 ml-1 h-full flex flex-col justify-center ${darkMode ? 'border-black' : 'border-gray-100'} ${activeChatId === chat.$id ? 'border-transparent' : ''}`}>
            <div className="flex justify-between items-baseline">
              <h3 className="font-semibold text-[15px] truncate dark:text-white">{chat.name}</h3>
